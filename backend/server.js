...
const GOOGLE_REDIRECT_URI = 'http://192.168.2.125/api/auth/google/callback';
...
const MICROSOFT_REDIRECT_URI = 'http://192.168.2.125/api/auth/microsoft/callback';
...
const initializeDatabase = async () => {
  try {
    ...
    const adminEmail = 'admin@192.168.2.126';
    ...
    if (existingAdmin.rows.length === 0) {
      ...
      console.log('Default admin user created: admin@192.168.2.126 / admin123');
    }
...
app.get('/api/auth/google/callback', async (req, res) => {
  try {
    ...
    if (!code) {
      return res.redirect('http://192.168.2.126/auth?error=google_auth_failed');
    }
    ...
    res.redirect('http://192.168.2.126/auth?success=google_login');
    
  } catch (error) {
    console.error('Google OAuth error:', error);
    res.redirect('http://192.168.2.126/auth?error=google_auth_failed');
  }
});
...
app.get('/api/auth/microsoft/callback', async (req, res) => {
  try {
    ...
    if (!code) {
      return res.redirect('http://192.168.2.126/auth?error=microsoft_auth_failed');
    }
    ...
    res.redirect('http://192.168.2.126/auth?success=microsoft_login');
    
  } catch (error) {
    console.error('Microsoft OAuth error:', error);
    res.redirect('http://192.168.2.126/auth?error=microsoft_auth_failed');
  }
});
...
const startServer = async () => {
  ...
  app.listen(PORT, '0.0.0.0', () => {
    ...
    console.log('Default admin: admin@192.168.2.126 / admin123');
  });
};
