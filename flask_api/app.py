...
try:
    # Read the service account JSON from the file baked into the image
    with open("/app/test-479119-ca31806eb033.json") as f:
...
VISION_API_KEY = "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"  # <-- CHANGE THIS TO YOUR ACTUAL API KEY
...
def get_time_series_data_sync(geometry: List[List[float]], start_date: str, end_date: str, 
                           selected_indices: List[str]) -> Dict[str, Any]:
    """
    Synchronous version of get_time_series_data - runs in thread pool.
    """
    try:
        # Create AOI geometry
...
def get_time_series_data_threaded(geometry: List[List[float]], start_date: str, end_date: str, 
                              selected_indices: List[str]):
    """
    Thread-based wrapper for get_time_series_data that runs in thread pool.
    This prevents blocking the main Flask worker.
    """
    ...
    # Get result (this will block, but worker can handle other requests in thread pool)
    try:
        result = future.result(timeout=300)  # 5 minute timeout
        end_time = time.time()
        logger.info(f"Threaded time series calculation completed in {end_time - start_time:.2f} seconds")
        return result
    except Exception as e:
        end_time = time.time()
        logger.error(f"Thread pool execution error after {end_time - start_time:.2f} seconds: {e}")
        return {
            'success': False,
            'error': str(e)
        }

def download_satellite_image_sync(wms_url: str, image_path: str) -> bool:
    """Synchronous satellite image download - runs in thread pool."""
    try:
...
def download_satellite_image_threaded(wms_url: str, image_path: str) -> bool:
    ...
    try:
        ...
        end_time = time.time()
        duration = end_time - start_time
        logger.error(f"❌ Threaded WMS download failed after {duration:.2f} seconds: {e}")
        return False

def download_comparison_images_sync(image1_url: str, image2_url: str, image1_path: str, image2_path: str) -> bool:
    """Synchronous comparison images download - runs in thread pool."""
    try:
        logger.info(f"Downloading comparison images synchronously")
        
        # Download first image
        response1 = requests.get(image1_url, timeout=60)
        ...
        response2 = requests.get(image2_url, timeout=60)
        response2.raise_for_status()
        
        with open(image2_path, 'wb') as f:
            f.write(response2.content)
        
        logger.info(f"Successfully downloaded both comparison images")
        return True
        
    except Exception as e:
        logger.error(f"Error downloading comparison images: {e}")
        return False

def download_comparison_images_threaded(image1_url: str, image2_url: str, image1_path: str, image2_path: str) -> bool:
    """Download comparison images using thread pool to avoid blocking worker."""
    try:
        start_time = time.time()
        logger.info(f"🛰 Starting threaded comparison images download")
        
        # Submit blocking download to thread pool
        future = wms_executor.submit(download_comparison_images_sync, image1_url, image2_url, image1_path, image2_path)
        result = future.result(timeout=180)  # 3 minute timeout
        
        end_time = time.time()
        duration = end_time - start_time
        logger.info(f"✅ Threaded comparison download completed in {duration:.2f} seconds")
        ...
        end_time = time.time()
        duration = end_time - start_time
        logger.error(f"❌ Threaded comparison download failed after {duration:.2f} seconds: {e}")
        return False
...
def calculate_time_series():
    ...
    try:
        ...
        # Run the blocking operation in thread pool to avoid blocking the worker
        result = get_time_series_data_threaded(geometry, start_date, end_date, selected_indices)
...
def download_comparison_images():
    ...
    try:
        ...
        image1_url_frontend = f"http://192.168.2.124/comparison-images/{image1_filename}"
        image2_url_frontend = f"http://192.168.2.124/comparison-images/{image2_filename}"
