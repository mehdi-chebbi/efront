...
export default function AdminDashboard() {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch('http://192.168.2.125/api/admin/users', {
        credentials: 'include'
      })
  ...
  const fetchLogs = async () => {
    try {
      const response = await fetch('http://192.168.2.125/api/admin/login-logs', {
        credentials: 'include'
      })
      const data = await response.json()
      if (data.success) {
  ...
  const handleSaveUser = async () => {
    ...
    try {
      const response = await fetch(`http://192.168.2.125/api/admin/users/${editingUser.id}`, {
        method: 'PUT',
        headers: {
  ...
  const handleDeleteUser = async (userId: number) => {
    ...
    try {
      const response = await fetch(`http://192.168.2.125/api/admin/users/${userId}`, {
        method: 'DELETE',
        credentials: 'include'
      })
