...
export default function MapComponent({ className = '' }: MapProps) {
  ...
  const handleLoadWmsLayer = async (params: {
    ...
    const imageUrl = `https://sh.dataspace.copernicus.eu/ogc/wms/3bd4722b-13ea-4bab-9825-4f5f4f71240c?` +
...
// Handle AI analysis with streaming - now unified with chat
  const handleAnalyzeWithAI = async () => {
    ...
    try {
      ...
      if (imageMetadata) {
        ...
        const wmsUrl = `https://sh.dataspace.copernicus.eu/ogc/wms/3bd4722b-13ea-4bab-9825-4f5f4f71240c?` +
        ...
        const response = await fetch("http://192.168.2.124/api/vision/analyze_satellite_stream", {
      ...
      setConversationHistory(prev => [...prev, {
        ...
        content: "❌ Failed to connect to AI service. Please make sure Flask API is running on 192.168.2.124 and API key is configured."
  ...
  const handleSaveChat = async () => {
    ...
    try {
      ...
      const sessionResponse = await fetch('http://192.168.2.125/api/chat/sessions', {
        method: 'POST',
      ...
      for (const message of conversationHistory) {
        const messageResponse = await fetch(`http://192.168.2.125/api/chat/sessions/${sessionId}/messages`, {
          method: 'POST',
          headers: { 'Content-Type': 'application/json' },
          credentials: 'include',
          body: JSON.stringify({
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const response = await fetch('http://192.168.2.124/api/vision/chat_stream', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const handleLoadStatistics = async (params: {
    ...
    try {
      const response = await fetch('http://192.168.2.124/api/time_series', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      alert('Failed to load statistics data. Please make sure the Flask API is running on 192.168.2.124')
    } finally {
  ...
  const handleLoadComparison = async (params: {
    ...
    try {
      ...
      const response = await fetch('http://192.168.2.124/api/comparison/download', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
      ...
      <SatelliteLegend 
        activeLayer={imageMetadata?.layer ? (() => {
          ...
          const layerMap: Record<string, string> = {
            ...
            'BAI (Burn Area Index)': 'BAI',
            'EVI (Enhanced Vegetation Index)': 'EVI',
            'False color (urban)': 'FALSE_COLOR_URBAN',
            'False color (vegetation)': 'FALSE_COLOR',
            'NDWI (Normalized Difference Water Index)': 'NDWIIII',
            'SWIR': 'SWIR',
