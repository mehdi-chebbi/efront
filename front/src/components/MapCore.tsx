...
export default function MapCore({ 
  ...
  useEffect(() => {
    baseMaps.current = {
      'NDVI': L.tileLayer.wms('https://sh.dataspace.copernicus.eu/ogc/wms/3bd4722b-13ea-4bab-9825-4f5f4f71240c', {
        layers: 'NDVI-L2A',
        format: 'image/png',
        transparent: true,
  ...
  useEffect(() => {
    ...
    // Add zoom control (hidden - we'll use custom buttons)
    // L.control.zoom({ position: 'topright' }).addTo(map)
    console.log('Zoom control hidden (using custom buttons)')
    ...
    // Add draw control (hidden - we'll use custom buttons)
    try {
      const drawControl = new L.Control.Draw({
        position: 'topright',
        edit: { featureGroup: drawnItemsRef.current },
        draw: {
          polygon: false, // Hide all default drawing tools
          rectangle: false,
          circle: false,
          marker: false,
          polyline: false,
      ...
      // Don't add the control to map - we'll use custom buttons
      console.log('Draw control created but hidden (using custom buttons)')
    } catch (error) {
  ...
  const importShapefile = () => {
    ...
    input.type = 'file'
    input.accept = '.shp,.zip,.geojson,.json'
    ...
    input.onchange = async (e) => {
      ...
      try {
        onStatusUpdate('Loading shapefile...')
        ...
        if (file.name.toLowerCase().endsWith('.zip')) {
          ...
          geojsonData = Array.isArray(result) ? result[0] : result
        } else if (file.name.toLowerCase().endsWith('.shp')) {
          // For individual .shp files, we need to handle differently
          // shpjs typically expects ZIP files, but we can try
          const arrayBuffer = await file.arrayBuffer()
          try {
            const result = await shp(arrayBuffer)
            geojsonData = Array.isArray(result) ? result[0] : result
          } catch (err) {
            alert('Shapefile (.shp) files need to be in a ZIP file with their accompanying .dbf, .shx files. Please zip all shapefile files together and try again.')
            onStatusUpdate('Shapefile import failed')
            return
          }
          ...
          alert('Please select a valid shapefile (.zip), GeoJSON (.geojson), or JSON (.json) file')
          onStatusUpdate('Invalid file format')
          return
        }
        ...
        if (mapInstanceRef.current && drawnItemsRef.current) {
          ...
          // Add each feature to the drawn items layer
          geojsonLayer.eachLayer((layer) => {
            drawnItemsRef.current?.addLayer(layer)
          })
          
          // Fit map to the imported shape bounds
          if (geojsonLayer.getBounds().isValid()) {
            mapInstanceRef.current.fitBounds(geojsonLayer.getBounds())
            
            // Trigger draw complete callback
            onDrawComplete(geojsonLayer.getBounds(), 'polygon')
            onDrawCreated?.()
          }
          
          onStatusUpdate(`Shapefile imported: ${file.name}`)
          console.log('Shapefile imported successfully:', geojsonData)
        }
        
      } catch (error) {
        console.error('Error importing shapefile:', error)
        alert('Failed to import shapefile. Please make sure the file is a valid shapefile ZIP or GeoJSON file.')
        onStatusUpdate('Shapefile import failed')
      }
    }
