...
const testSaveChat = async () => {
  ...
  try {
    ...
    for (let i = 0; i < conversationHistory.length; i++) {
      ...
      const messageResponse = await fetch(`http://192.168.2.125/api/chat/sessions/${sessionData.session.id}/messages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
    ...
    console.log('\n💡 Make sure:')
    console.log('   1. Authentication server is running on 192.168.2.125')
