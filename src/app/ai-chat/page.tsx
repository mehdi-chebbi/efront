...
export default function AIChatPage() {
  ...
  const fetchSessions = async () => {
    try {
      const response = await fetch('http://api.misbar.africa/api/chat/sessions', {
        credentials: 'include'
      })
  ...
  const loadSession = async (session: ChatSession) => {
    ...
    try {
      const response = await fetch(`http://api.misbar.africa/api/chat/sessions/${session.id}/messages`, {
        credentials: 'include'
      })
  ...
  const deleteSession = async (sessionId: number, e: React.MouseEvent) => {
    ...
    try {
      const response = await fetch(`http://api.misbar.africa/api/chat/sessions/${sessionId}`, {
        method: 'DELETE',
        credentials: 'include'
      })
  ...
  const saveMessage = async (message: Message) => {
    ...
    try {
      const response = await fetch(`http://api.misbar.africa/api/chat/sessions/${currentSession.id}/messages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
  ...
  const handleSendMessage = async () => {
    ...
    try {
      ...
      const errorMessage: Message = {
        ...
        role: 'assistant',
        content: '❌ Failed to connect to AI service. Please make sure Flask API is running on chat.misbar.africa.',
        timestamp: new Date()
      }
