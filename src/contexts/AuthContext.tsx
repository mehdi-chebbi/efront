...
export const AuthProvider: React.FC<AuthProviderProps> = ({ children }) => {
  ...
  const checkAuth = async () => {
    try {
      const response = await fetch('http://api.misbar.africa/api/auth/me', {
        credentials: 'include'
      })
  ...
  const login = async (email: string, password: string) => {
    try {
      const response = await fetch('http://api.misbar.africa/api/auth/login', {
        method: 'POST',
        headers: {
  ...
  const register = async (userData: {
    ...
    try {
      const response = await fetch('http://api.misbar.africa/api/auth/register', {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
        },
  ...
  const logout = async () => {
    try {
      await fetch('http://api.misbar.africa/api/auth/logout', {
        method: 'POST',
        credentials: 'include'
      })
    } catch (error) {
