...
const testSaveChat = async () => {
  ...
  try {
    ...
    for (let i = 0; i < conversationHistory.length; i++) {
      ...
      const messageResponse = await fetch(`http://api.misbar.africa/api/chat/sessions/${sessionData.session.id}/messages`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        credentials: 'include',
        body: JSON.stringify({
    ...
    console.log(`📱 You can now view this chat at: http://misbar.africa/ai-chat`)
    
  } catch (error) {
    console.error('❌ Test FAILED:', error.message)
    console.log('\n💡 Make sure:')
    console.log('   1. Authentication server is running on api.misbar.africa')
